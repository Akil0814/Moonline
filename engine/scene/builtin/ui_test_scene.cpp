...
void UiTestScene::rebuild_ui()
{
    ...
    auto animation = std::make_unique<UiAnimation>(elysia::core::Rect{ 0,0,120,120 }); if (!animation->set_engine_animation(*cache,"engine.test.idle")) throw std::logic_error("UiTestScene could not bind engine.test.idle."); animation->play(); media_section->add_back(std::move(animation));
