...
ApplicationFontSettings settings_with_size(
    ...
    int point_size)
{
    ...
    settings.ui.typography = ApplicationTypographyProfile(sizes);
    settings.floating_number.source = effect_source;
    settings.floating_number.point_size = point_size;
    return settings;
}
    ...
    bool include_japanese)
{
    require(resources.load_font(
        ...
        20), "FontResolver tests must load the Latin project font");
    require(resources.load_font(
        "ui.zh_hans.20",
        font_root / "fusion-pixel-10px-proportional-zh_hans.ttf",
        20), "FontResolver tests must load the Simplified Chinese project font");
    if (include_japanese)
    {
        require(resources.load_font(
            "ui.ja.20",
            font_root / "fusion-pixel-10px-proportional-ja.ttf",
            20), "FontResolver tests must load the Japanese project font");
    }
}
...
void test_atomic_project_activation(FontResolverFixture& fixture)
{
    ...
    load_project_fonts(fixture.resources(),font_root,false);
    ...
    require(fixture.resources().load_font(
        "ui.ja.20",
        font_root / "fusion-pixel-10px-proportional-ja.ttf",
        20), "FontResolver tests must complete the project font set");
...
void test_invalid_configuration(FontResolverFixture& fixture)
{
    ...
    const auto invalid = resolver.configure(
        ...
        languages);
    require(!invalid
        && invalid.error().code == FontResolveErrorCode::InvalidPointSize,
        "FontResolver must reject non-positive configured point sizes");

    ApplicationFontSettings invalid_source_settings;
    invalid_source_settings.ui.source =
        static_cast<ApplicationFontSource>(255);
    const auto invalid_source = resolver.configure(
    ...
    require(!invalid_source
        ...
        "FontResolver must reject unknown font sources");
}
